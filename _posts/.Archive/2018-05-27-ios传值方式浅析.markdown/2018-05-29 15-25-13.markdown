...
3. **代理传值**  
代理传值(场景)一般用于逆向传值,即第二个界面传值给第一个界面,ViewController页面push到SecondViewController页面,如果SecondViewController页面的信息想回传(回调)到ViewController页面,用代理传值，其中SecondViewController定义协议和声明代理,ViewController确认并实现代理,ViewController作为SecondViewController的代理
