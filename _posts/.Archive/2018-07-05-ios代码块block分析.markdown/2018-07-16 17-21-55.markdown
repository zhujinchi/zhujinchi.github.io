...
代码块Block是苹果在iOS4开始引入的对C语言的扩展,用来实现匿名函数的特性,Block是一种特殊的数据类型,其可以正常定义变量.&nbsp作为参数.&nbsp作为返回值,特殊地,Block还可以保存一段代码,在需要的时候调用,目前Block已经广泛应用于iOS开发中,常用于GCD.&nbsp动画.&nbsp排序及各类回调
 Block的声明和赋值只是保存了一段代码段，必须调用才能执行代码块。      

#1.&nbsp关于Block变量的声明.&nbsp赋值和调用  
##1.1.&nbspBlock变量的声明
```objc    
...
#5.&nbspBlock内访问__block修饰的局部变量

#6.&nbspBlock内访问全局变量

#7.&nbspBlock内访问静态变量

#8.&nbspBlock在MRC及ARC下的内存管理
